...
namespace luster::gfx
{
    void Mesh::createCube(Device& device)
    {
        ...
        const V verts[] = {
            ...
            {-0.5f, -0.5f, -0.5f, 1,0,1}, { 0.5f, -0.5f, -0.5f, 0,1,1}, { 0.5f,  0.5f, -0.5f, 1,1,1}, { -0.5f,  0.5f, -0.5f, 0.5,0.5,0.5}
        };
        ...
        BufferCreateInfo vbi{}; vbi.size = sizeof(verts); vbi.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT; vbi.properties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
        vertexBuffer_->create(device, vbi);
        vertexBuffer_->upload(device, verts, sizeof(verts));

        indexBuffer_ = std::make_unique<Buffer>();
        BufferCreateInfo ibi{}; ibi.size = sizeof(indices); ibi.usage = VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT; ibi.properties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
    ...
    {
        if (indexBuffer_) { indexBuffer_->cleanup(device); indexBuffer_.reset(); }
        if (vertexBuffer_) { vertexBuffer_->cleanup(device); vertexBuffer_.reset(); }
        indexCount_ = 0;
        vertexLayout_ = VertexLayout{};
    }
