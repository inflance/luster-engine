...
namespace luster::gfx
{
    ...
    {
        cleanup(device);
        width_ = info.width; height_ = info.height; format_ = info.format;
    ...
    void Image::cleanup(const Device& device)
    {
        ...
        image_ = VK_NULL_HANDLE; memory_ = VK_NULL_HANDLE; view_ = VK_NULL_HANDLE;
        width_ = height_ = 0; format_ = VK_FORMAT_UNDEFINED;
    }
}
