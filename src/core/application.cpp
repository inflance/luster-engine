...
namespace luster
{
	// ---- Helpers and structures (migrated from main.cpp) ----
	...
	struct Application::VulkanState
	{
	...
	void Application::createInstance(SDL_Window* window, VulkanState& vk)
	{
	...
	void Application::pickDevice(VulkanState& vk)
	{
		uint32_t count = 0;
	...
	void Application::createDevice(VulkanState& vk)
	{
		float priority = 1.0f;
	...
	void Application::createSwapchainAndViews(SDL_Window* window, VulkanState& vk)
	{
	...
	void Application::createRenderPass(VulkanState& vk)
	{
	...
	void Application::createPipeline(VulkanState& vk)
	{
	...
	void Application::createFramebuffers(VulkanState& vk)
	{
	...
	void Application::createCommandsAndSync(VulkanState& vk)
	{
		...
		if (r != VK_SUCCESS) throw std::runtime_error(
	...
	void Application::cleanupSwapchain(VulkanState& vk)
	{
	...
	void Application::recreateSwapchain(SDL_Window* window, VulkanState& vk)
	{
		int w = 0, h = 0;
		SDL_GetWindowSize(window, &w, &h);
	...
	void Application::recordAndSubmitFrame(VulkanState& vk, uint32_t imageIndex)
	{
	...
	void Application::init()
	{
		...
		vk_state_ = new VulkanState();

		try
		{
			createInstance(window_->sdl(), *vk_state_);
			spdlog::info("Vulkan instance created");

			pickDevice(*vk_state_);
			spdlog::info("Physical device selected");

			createDevice(*vk_state_);
			spdlog::info("Logical device created");

			createSwapchainAndViews(window_->sdl(), *vk_state_);
			spdlog::info("Swapchain created: {} images, {}x{}, format {}",
			             static_cast<int>(vk_state_->swapImageViews.size()),
			             vk_state_->swapExtent.width, vk_state_->swapExtent.height,
			             static_cast<int>(vk_state_->swapFormat));

			createRenderPass(*vk_state_);
			createPipeline(*vk_state_);
			createFramebuffers(*vk_state_);
			createCommandsAndSync(*vk_state_);
		}
	...
	void Application::mainLoop()
	{
		...
		while (running)
		{
			running = window_->pollEvents(vk_state_->framebufferResized);

			if (vk_state_->framebufferResized)
			{
				vk_state_->framebufferResized = false;
				try
				{
					recreateSwapchain(window_->sdl(), *vk_state_);
				}
				catch (const std::exception& ex)
				{
			...
			VkResult r = vkWaitForFences(vk_state_->device, 1, &vk_state_->inFlight, VK_TRUE, UINT64_C(1'000'000'000));
			if (r != VK_SUCCESS)
			{
			...
			vkResetFences(vk_state_->device, 1, &vk_state_->inFlight);

			// Acquire image
			uint32_t imageIndex = 0;
			r = vkAcquireNextImageKHR(vk_state_->device, vk_state_->swapchain, UINT64_MAX, vk_state_->semImageAvailable,
			                          VK_NULL_HANDLE, &imageIndex);
			if (r == VK_ERROR_OUT_OF_DATE_KHR)
			{
				vk_state_->framebufferResized = true;
				continue;
			}
			...
			try
			{
				recordAndSubmitFrame(*vk_state_, imageIndex);
			}
			catch (const std::exception& ex)
			{
			...
			pi.pWaitSemaphores = &vk_state_->semRenderFinished;
			pi.swapchainCount = 1;
			pi.pSwapchains = &vk_state_->swapchain;
			pi.pImageIndices = &imageIndex;
			r = vkQueuePresentKHR(vk_state_->presentQueue, &pi);
	...
	void Application::cleanup()
	{
		...
		// Wait for device to complete all operations
		if (vk_state_->device) vkDeviceWaitIdle(vk_state_->device);

		// Cleanup synchronization objects
		if (vk_state_->inFlight) vkDestroyFence(vk_state_->device, vk_state_->inFlight, nullptr);
		if (vk_state_->semRenderFinished) vkDestroySemaphore(vk_state_->device, vk_state_->semRenderFinished, nullptr);
		if (vk_state_->semImageAvailable) vkDestroySemaphore(vk_state_->device, vk_state_->semImageAvailable, nullptr);

		// Cleanup command objects
		if (vk_state_->cmdBuf) vkFreeCommandBuffers(vk_state_->device, vk_state_->cmdPool, 1, &vk_state_->cmdBuf);
		if (vk_state_->cmdPool) vkDestroyCommandPool(vk_state_->device, vk_state_->cmdPool, nullptr);

		// Cleanup swapchain and related objects
		cleanupSwapchain(*vk_state_);

		// Cleanup device and surface
		if (vk_state_->device) vkDestroyDevice(vk_state_->device, nullptr);
		if (vk_state_->surface) vkDestroySurfaceKHR(vk_state_->instance, vk_state_->surface, nullptr);

		// Destroy debug messenger before destroying instance
		destroyDebugMessenger(vk_state_->instance, vk_state_->debugMessenger);

		if (vk_state_->instance) vkDestroyInstance(vk_state_->instance, nullptr);
