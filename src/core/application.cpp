...
namespace luster
{
	...
	void Application::init()
	{
		...
		try {
			...
			createRenderPass(*m_vk);
			createPipeline(*m_vk);
			createFramebuffers(*m_vk);
			createCommandsAndSync(*m_vk);
	...
	void Application::mainLoop()
	{
		...
		while (running)
		{
			...
			VkResult r = vkWaitForFences(m_vk->device, 1, &m_vk->inFlight, VK_TRUE, UINT64_C(1'000'000'000));
			if (r != VK_SUCCESS) {
				spdlog::error("vkWaitForFences failed: {}", vk_err(r));
				running = false;
				break;
			}
			vkResetFences(m_vk->device, 1, &m_vk->inFlight);

			// Acquire image
			uint32_t imageIndex = 0;
			r = vkAcquireNextImageKHR(m_vk->device, m_vk->swapchain, UINT64_MAX, m_vk->semImageAvailable, VK_NULL_HANDLE, &imageIndex);
			...
			try {
				recordAndSubmitFrame(*m_vk, imageIndex);
			} catch (const std::exception& ex) {
			...
			pi.pWaitSemaphores = &m_vk->semRenderFinished;
			pi.swapchainCount = 1;
			pi.pSwapchains = &m_vk->swapchain;
			pi.pImageIndices = &imageIndex;
			r = vkQueuePresentKHR(m_vk->presentQueue, &pi);
	...
	void Application::cleanup()
	{
		...
		if (m_vk->device) vkDeviceWaitIdle(m_vk->device);

		// Cleanup synchronization objects
		if (m_vk->inFlight) vkDestroyFence(m_vk->device, m_vk->inFlight, nullptr);
		if (m_vk->semRenderFinished) vkDestroySemaphore(m_vk->device, m_vk->semRenderFinished, nullptr);
		if (m_vk->semImageAvailable) vkDestroySemaphore(m_vk->device, m_vk->semImageAvailable, nullptr);

		// Cleanup command objects
		if (m_vk->cmdBuf) vkFreeCommandBuffers(m_vk->device, m_vk->cmdPool, 1, &m_vk->cmdBuf);
		if (m_vk->cmdPool) vkDestroyCommandPool(m_vk->device, m_vk->cmdPool, nullptr);

		// Cleanup swapchain and related objects
		cleanupSwapchain(*m_vk);

		// Cleanup device and surface
		if (m_vk->device) vkDestroyDevice(m_vk->device, nullptr);
		if (m_vk->surface) vkDestroySurfaceKHR(m_vk->instance, m_vk->surface, nullptr);
		
		// Destroy debug messenger before destroying instance
		destroyDebugMessenger(m_vk->instance, m_vk->debugMessenger);
		
		if (m_vk->instance) vkDestroyInstance(m_vk->instance, nullptr);
